...
- **`?nethpraise=0`** (card 87310afa): restore the pre-card DOWNWARD-ONLY clamp on a client
  ...
  over-predictions (its bullets run the real `HitBy` on puppets, while the host's own per-entity
  35ms hittimer may refuse those same hits) were permanent, so its copy ratcheted below the host's
  and could claim a boss dead while the host's still had HP. **Another deliberate bug
  reproduction** (the `?nethitstop=1` / `?netstaleguard=0` / `?netaimease=0` idiom) and IN
  `DebugFlags.Active` for that reason; like those two it turns a shipped FIX off, so it DEFAULTS
  TRUE and `Active` tests its negation. It is also the mutation control for the raise legs in
  `eaNetEntity()` / `eaNetSnap()`. Cosmetic cost either way (hp's only puppet-side readers are the
  colorize redden and `BattleSkull`'s hue); no protocol change. Details: net CLAUDE.md.
...
- **AI bench** (card f4d1721f): `?aibench` turns on telemetry for the `ControlDevice.AI` ships —
  ...
  ?aithreatlead= ?aibossbias= ?aiaim= ?aifieldpow= ?aitristrength= ?ainoisefloor=
  ?aiseekdeadzone= ?aiseekresume= ?aiseekpowerup= ?aiseekapproach= ?aipowerupreach=
  ?aievade= ?aicone= ?aiwedge= ?aiconelead= ?aiconemaxlen= ?aisweptmax= ?aitopedgepx=
  ?aitopedgestrength= ?ailazerpx= ?ailazerstrength= ?ailazerdodge= ?aispares=
  ?aisparefair= ?aiseeklog` (the owner-redesign set, fable-iterative rep 1 -- the old mesa/field/asteroid/lane
  knob families are DELETED; web CLAUDE.md's AI banner has the redesign summary). Pair with `?aiplayer`.
...
- **`?minelog`** (card 745728f9): make every `StarMine` (Level 3's space mine, aka death star)
  report each lock-on, release and death on a `[mine]` line. Every line carries a timestamp and
  the distance and AGE of the nearest recorded player death spot; a release or a death also
  carries the `reason=` it happened, and a detonation additionally carries `live=`, the distance
  to the nearest LIVE ship. None of that is visible in a frame: a locked mine and a free one draw the same
