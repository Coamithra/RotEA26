...
> **STATUS 2026-08-23 — the epic is COMPLETE (11.7–11.11 all SHIPPED), and this doc was written 2026-07-24 against a codebase that has since moved. The architecture below STANDS (star topology, `PeerChannel`, unified ship paths, negotiated signaling capacity, the listed-session match-end shape); the following specifics are corrected here rather than rewritten in place:**
>
> - **Stage 11.11 SHIPPED (card `6fb406bc`, no protocol bump):** the hardening pass. The relayed-channel interp delay landed as the §"Topology" cost paragraph proposed (~150 ms, `ShipFlagRelayed` + `RelayedInterpDelayMs`, fixed rather than jitter-derived); the host-uplink estimate below was MEASURED on the four-process rig — ~20–29 KB/s payload at N=4 with all four ships live (the §"Host bandwidth" estimate of ~33 KB/s confirmed sane; the ×2–3 header multiplier stands, so budget ~60–90 KB/s framed) via new `txB/rxB/txBps/rxBps` fields on the `[net]` line; webrtc.js gained `bufferedAmount` back-pressure (stream drops past a 16 KB backlog, reliable never drops); and `tools/sim/net_npeer_smoke.py` is the full four-machine star with the bandwidth soak and a two-survivor departure. Details: `Compat/Net/CLAUDE.md` → "STAGE 11.11 HARDENING". What remains is real-world only: no four-real-browsers WebRTC playtest has run.
>
...
> - **TURN is decided: STUN-only stays** (owner decision, 2026-08-21; sharpened 2026-08-23: **no self-hosted coturn ever** — the shared VPS has no bandwidth budget for relaying game traffic). §"Topology"'s go/no-go is resolved as a deferred follow-up card (filed at 11.11's close), gated on real-world lobby-formation failure reports; if those land, the candidates are a free-tier managed TURN service in the ICE config or accepting the failure rate with its clean error. Note "use one of the players as the relay" is already half-true — the star makes the HOST the relay for everything in-lobby; TURN addresses the different case of a pair that cannot complete ICE at all, and a peer-as-TURN fallback would be bespoke netcode for the ~10–15% case that only helps when a third, well-connected player exists.
