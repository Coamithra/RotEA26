...
Derived by reading `src_decompiled/EvilAliens/PlayerShip.cs` (`DoAIMove` 619–1166, `DoAIFire` 508–618) side by side with this branch's `Game/EvilAliens/PlayerShip.cs`. Every claim below was checked in both sources, not quoted from docs.

**Lap-2 owner rulings (this branch, superseding the states described below):** the repel/attractor floor split is GONE — one blanket 0.2 floor on the final sum, 2008's own line at 2008's place; the seek arrival deadzone is replaced by **hysteresis** (park ≤ 8px, resume ≥ 20px; `?aiseekdeadzone=`/`?aiseekresume=`); the threat/beam field curve **defaults to the 2008 plateau** (`?aifieldcurve=port` restores the spike); **velocity cones, the lane wedge and closest-approach evade are baked OFF** (code kept; `?aicone=1`/`?aiwedge=1`/`?aievade=1` re-arm, probes pin the dormant paths); the **powerup edge yield is reverted** (bandaid on a bandaid); the **per-difficulty skill ladder is retired** (one fixed skill — aim π/12, field base 150 — at every tier, as 2008). Still standing from the port: size-scaled field *range*, the top-edge band (170/20, awaiting a ruling once the classic curve has been seen), boss approach, big-UFO sparing, lane escapes, wall nav, world-model additions.
