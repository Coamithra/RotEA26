...
1. **Clamp the world-visible dt in `Game1.UpdateCore`** to `DefaultMaxWorldDtMs = 100` (6 sim-frames):
   ...
   - Decision extracted as a pure static (`Game1.ClampedWorldDtTicks`) so `logic_probe` covers it with no rig.
   - The leading tick of an over-threshold run says so — `[maxdt] clamped a <raw>ms tick to <max>ms`, or `passed ... whole` when the flag or a net session let it through (the `[hitch]` watchdog's edge-detection precedent; the flag changes the drag, never the measurement). The delivered ms is read off the reassigned `gameTime`, so a print-without-apply mutant fails the probe.
...
4. **Probes**: `tools/headless/probes/maxdt_clamp.txt` (default boot: `stepdt 500` → the exact `clamped a 500ms tick to 100ms` line; ordinary frames print nothing) + `maxdt_clamp_off.txt` (`?maxdt=0` boot: the `passed ... whole` spelling, no `clamped` line, and the flag in the `Active` dump). A boot pair because the flag parses at boot. NOTE `eval WorldClock` cannot be the observable — `WorldTime.Advance` caps its own dt at 0.1 s, so the clock reads the same on both builds (an earlier cut of the probe had that vacuous leg). `logic_probe`: `ProbeMaxWorldDt` (default/boundary/net-exempt/override/off cases, against the real static) + a `?maxdt=` row in the flag-rejection sweep.
