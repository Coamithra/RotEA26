...
- **`MsgShipState` and `MsgFriendState` grow two trailing bytes** (31 -> 33): `[asplodeBits:1][bounceBits:1]`. Bit `i` = the roll outcome of the shot whose cumulative count is `ShotCount - i` (bit 0 = the newest counted shot). `NetMaxCatchUpShots` is 6, so an 8-bit ring covers every shot a packet can owe, including after loss. Protocol **v20 -> v21** -- a real bump like v12: the decoders' length gates move 31 -> 33 with the fixed-width layouts, so a v20 peer's ship frames would be refused wholesale (frozen puppet), not gracefully re-rolled.
