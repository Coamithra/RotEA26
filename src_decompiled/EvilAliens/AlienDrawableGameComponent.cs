...
public abstract class AlienDrawableGameComponent : DrawableGameComponent, ICollidable, IComponentWatcher
{
	...
	protected Vector2 SpeedVector
	{
		...
		set
		{
			...
			_speed = ((Vector2)(ref value)).Length();
	...
	protected void Move(float? direction, GameTime gameTime)
	{
		...
		_speed = MathHelper.Clamp(((Vector2)(ref v)).Length(), _minimumSpeed, _maximumSpeed);
	}
	...
	private void drawWithoutInterpolation()
	{
		...
		if (flag)
		{
			...
			spriteBatch.fadeEffect.Value = ((Color)(ref color)).ToVector4();
		}
	...
	private Rectangle getFrameRectangle(int framenr)
	{
		...
		((Rectangle)(ref result))._002Ector(num2 * (num3 + separatingspace), num * (num4 + separatingspace), num3, num4);
		return result;
	}
	...
	private void drawWithInterpolation()
	{
		...
		case 2:
		{
			...
			((Color)(ref val2))._002Ector(new Vector4(1f, 1f, 1f, 1f - num2));
			Color val3 = default(Color);
			((Color)(ref val3))._002Ector(new Vector4(1f, 1f, 1f, num2));
		...
		case 1:
			...
			spriteBatch.interpolateEffect.Offset = new Vector2((float)(((Rectangle)(ref frameRectangle2)).Left - ((Rectangle)(ref frameRectangle)).Left), (float)(((Rectangle)(ref frameRectangle2)).Top - ((Rectangle)(ref frameRectangle)).Top)) / new Vector2((float)texture.Width, (float)texture.Height);
