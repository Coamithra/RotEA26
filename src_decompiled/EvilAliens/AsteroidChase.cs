...
internal class AsteroidChase : GameScene
{
	...
	protected override void PopulateEventList()
	{
		...
		AsteroidSpawner gameEvent = new AsteroidSpawner(((GameComponent)this).Game, 60f, 5f, startWithBig: true);
		eventList.AddEvent(gameEvent, halting: true);
		BonusSpawner bonusSpawner = new BonusSpawner(((GameComponent)this).Game, 60f, 1f, randomly: false);
		bonusSpawner.SetMars();
		eventList.AddEvent(bonusSpawner, halting: false);
		UfoSpawner ufoSpawner = new UfoSpawner(((GameComponent)this).Game, 60f, 8f, big: false);
		eventList.AddEvent(ufoSpawner, halting: true);
		ufoSpawner.SetupAsteroidChase();
		eventList.AddHalt();
		waitEvent = new WaitEvent(((GameComponent)this).Game, 4f);
