...
public class CastDisplayer : DrawableGameComponent, IComponentWatcher
{
	...
	private void AsplodeBrainBoss()
	{
		...
		((Vector2)(ref val))._002Ector(400f, 80f);
		UberExplosion(val);
		...
		bloodExplosion = BloodExplosion.NewExplosion(collection, ((GameComponent)this).Game);
		bloodExplosion.Setup(val, 7f, 6f, 0f, 0f);
		collection.Add((GameComponent)(object)bloodExplosion);
		bloodExplosion = BloodExplosion.NewExplosion(collection, ((GameComponent)this).Game);
		bloodExplosion.Setup(val, 7f, 6f, 0f, 0f);
	...
	public override void Draw(GameTime gameTime)
	{
		...
		if (spiderdeadtimer.Active)
		{
			...
			((Color)(ref val))._002Ector(new Vector4(1f, 1f, 1f, MathHelper.Lerp(0f, 1f, spiderdeadtimer.TimeLeft * 3f / stateTimer.Duration)));
		...
		else if (texture != null)
		{
			...
			{
				((Vector2)(ref val3))._002Ector(400f, 80f);
			}
			else
			{
				val3 = spawnposition;
			...
			if ((columns > 1) | (rows > 1))
			{
				...
				((Rectangle)(ref source))._002Ector(num4 * (num5 + separatingspace), num3 * (num6 + separatingspace), num5, num6);
		...
		float num7 = 0.5f;
		float num8 = 0.8f;
		float num9 = (float)((Rectangle)(ref General.SafeZone)).Bottom - MathHelper.Max((float)AButton.Height * num7, font.MeasureString("yo").Y * num8);
		...
		float num10 = (float)((Rectangle)(ref General.SafeZone)).Right - font.MeasureString("next").X * num8;
