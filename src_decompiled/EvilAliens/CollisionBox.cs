...
public class CollisionBox : ICollisionType
{
	...
	private bool TestCollisionLine(CollisionLine collisionLine)
	{
		...
		((BoundingBox)(ref val))._002Ector(new Vector3(TopLeft, 0f), new Vector3(BottomRight, 0f));
		Ray val2 = default(Ray);
		((Ray)(ref val2))._002Ector(new Vector3(collisionLine.Origin, 0f), new Vector3(collisionLine.DirectionalVector, 0f));
		if (((BoundingBox)(ref val)).Intersects(val2).HasValue && ((BoundingBox)(ref val)).Intersects(val2) < collisionLine.Length)
		{
			return true;
		}
		return false;
