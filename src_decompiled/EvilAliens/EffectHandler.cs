...
public class EffectHandler
{
	...
	private Color RGBtoHSV(Color color)
	{
		...
		Vector4 val = ((Color)(ref color)).ToVector4();
	...
	private Color HSVtoRGB(Color hsv_color)
	{
		...
		Vector4 val = ((Color)(ref hsv_color)).ToVector4();
		val.X *= 360f;
		Vector4 val2 = default(Vector4);
		val2.W = val.W;
		if (val.Y == 0f)
		...
		for (int i = 0; i < 256 / num; i++)
		{
			for (int j = 0; j < 256 / num; j++)
			{
				for (int k = 0; k < 256 / num; k++)
				{
					...
					array[i + j * 256 / num + k * 256 / num * 256 / num] = ((Color)(ref val)).PackedValue;
					Color val2 = HSVtoRGB(new Color((byte)(i * num), (byte)(j * num), (byte)(k * num)));
					array2[i + j * 256 / num + k * 256 / num * 256 / num] = ((Color)(ref val2)).PackedValue;
				}
			}
		}
		conversionRGBtoHSV = new Texture3D(graphicsDevice, 256 / num, 256 / num, 256 / num, 1, (TextureUsage)0, (SurfaceFormat)2);
		conversionRGBtoHSV.SetData<uint>(array);
		conversionHSVtoRGB = new Texture3D(graphicsDevice, 256 / num, 256 / num, 256 / num, 1, (TextureUsage)0, (SurfaceFormat)2);
