...
internal class MenuScene : Scene
{
		...
		: base(game)
	{
		...
		mainMenu = new MenuSubWithSkull(((GameComponent)this).Game);
		if (General.IsTrial)
		{
		...
		levelSelector = new SubMenuLevelChoice(((GameComponent)this).Game);
		...
		levelSelector.AddEntryEvent(levelSelector_levelSelected);
		confirmationMenu = new ConfirmationMenu(((GameComponent)this).Game, "Are you sure?\nThis will erase all progress..");
		...
		challengeSelector = new SubMenuLevelChoice(((GameComponent)this).Game);
		...
		challengeSelector.AddEntryEvent(challengeSelector_levelSelected);
		cheatsMenu = new MenuSub1(((GameComponent)this).Game);
		...
		optionsMenu = new MenuSub1(((GameComponent)this).Game);
		...
		colorsMenu = new MenuSub1(((GameComponent)this).Game);
		...
		difficultyMenu = new DifficultyMenu(((GameComponent)this).Game);
		...
		playtestMenu = new MenuSub1(((GameComponent)this).Game);
		...
		trailerMenu = new MenuSub1(((GameComponent)this).Game);
		...
		trailerScene = new TrailerScene(((GameComponent)this).Game);
		trailerScene.OnFinished += trailerScene_OnFinished;
		((DrawableGameComponent)this).DrawOrder = 1;
	}
	...
	{
		((Game1)(object)((GameComponent)this).Game).WantExit();
	}
	...
	private void drawButtonTips()
	{
		...
		float num = 0.5f;
		float num2 = 0.8f;
		float num3 = ((Rectangle)(ref General.SafeZone)).Left;
		float num4 = (float)((Rectangle)(ref General.SafeZone)).Bottom - MathHelper.Max((float)AButton.Height * num, font.MeasureString("yo").Y * num2);
		float num5 = num3 + (float)AButton.Width * num + font.MeasureString(" ").X * num2;
		float num6 = (float)((Rectangle)(ref General.SafeZone)).Right - font.MeasureString("select").X * num2;
	...
	private void CreateStar(bool moveit)
	{
		...
		{
			star = new Star(((GameComponent)this).Game as Game1, stargfx, origin, size, num2, num);
		}
		else
		{
			star = idleStars[0];
