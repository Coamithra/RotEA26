...
internal class OwnLevel : GameScene
{
	...
	{
		Wait(1f);
		MessageEvent messageEvent = new MessageEvent(((GameComponent)this).Game, "Get ready!", SoundManager.Texts.GetReady);
		eventList.AddEvent(messageEvent, halting: false);
		messageEvent.OnFinished += setspeed;
		Wait(2f);
		SkullSpawner gameEvent = new SkullSpawner(((GameComponent)this).Game, 0f, 2f, maze: true, bonusonly: false);
		eventList.AddEvent(gameEvent, halting: false);
		eventList.SetLastEventAsCheckPoint();
		StarMineSpawner starMineSpawner = new StarMineSpawner(((GameComponent)this).Game, 0f, 0.1f);
		...
		Walls gameEvent2 = new Walls(((GameComponent)this).Game, 2);
