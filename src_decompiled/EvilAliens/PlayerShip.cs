...
public class PlayerShip : AlienDrawableGameComponent
{
	...
	private void PlayerShip_OnDeath(object sender)
	{
		...
		PlayerShipSummon playerShipSummon = PlayerShipSummon.NewPlayerShipSummon(collection, ((GameComponent)this).Game);
		...
		if (!hasWon)
		{
			...
			else
			{
				...
				case ControlDevice.PadFour:
				{
					...
					if (((Vector2)(ref val)).LengthSquared() > 0.09f)
					{
						direction = input.LeftStick(i);
					}
					Vector2 val2 = input.RightStick(i);
					if (((Vector2)(ref val2)).LengthSquared() > 0.0025000002f)
					{
	...
	private void doBlast()
	{
		...
		if (Score.NrBombs(player) > 0)
		{
			...
			blast = Blast.NewBlast(collection, ((GameComponent)this).Game);
	...
	private void DoAIFire(GameTime gameTime)
	{
		...
		foreach (AlienDrawableGameComponent baddy in oracle.GetBaddies())
		{
			if (baddy is UFO || baddy is Braineroid || (baddy is Ball && ((Ball)baddy).IsConnected()) || baddy is JunkBoss || baddy is Boss || baddy is Spider || baddy is MarsBoss || baddy is DeathStar || baddy is ClassicBoss || baddy is BattleSkull || (baddy is FlyingSpider && baddy.Collides) || baddy is StarMine || (baddy is EvilSkull && !((EvilSkull)baddy).Fading) || baddy is SweepUFO)
			{
				...
				if (((Vector2)(ref val)).LengthSquared() < num2 && baddy.Position.X > 0f && baddy.Position.X < 800f && baddy.Position.Y > 0f && baddy.Position.Y < 600f)
				{
					Vector2 val2 = baddy.Position - base.Position;
					num2 = ((Vector2)(ref val2)).LengthSquared();
	...
	private void doAIBomb()
	{
		...
		foreach (AlienDrawableGameComponent baddy in oracle.GetBaddies())
		{
			...
			{
				Vector2 val = baddy.Position - base.Position;
				if (((Vector2)(ref val)).LengthSquared() <= num3 * num3)
				{
					num2++;
				}
			}
	...
	private void DoAIMove(ref Vector2 direction, GameTime gameTime)
	{
		...
		((Vector2)(ref position))._002Ector(float.MaxValue, float.MaxValue);
		float num4 = 0f;
		if (player == 0)
		{
		...
		foreach (AlienDrawableGameComponent baddy in oracle.GetBaddies())
		{
			if (isBlastable(baddy) && blast != null && blast.Collides)
			{
				...
				float num5 = ((Vector2)(ref val)).LengthSquared();
				Vector2 val2 = position - base.Position;
				if (num5 < ((Vector2)(ref val2)).LengthSquared())
				{
			...
			else
			{
				...
				if (baddy.GetCollisionType() is CollisionBox)
				{
					...
					num9 = ((Vector2)(ref val3)).Length() - ((CollisionBox)baddy.GetCollisionType()).Width / 2f * (float)Math.Sqrt(2.0);
				}
				else if (baddy.GetCollisionType() is CollisionMultibox)
				{
					Vector2 val4 = base.Position - baddy.Position;
					num9 = ((Vector2)(ref val4)).Length() - ((CollisionMultibox)baddy.GetCollisionType()).Items[0].Width / 2f * (float)Math.Sqrt(2.0);
				}
				else if (baddy.GetCollisionType() is CollisionSimpleCircle)
				{
					...
					num9 = MathHelper.Clamp(((Vector2)(ref val5)).Length() - radius, 0f, 1000f);
				}
				else
				{
					Vector2 val6 = base.Position - baddy.Position;
					num9 = ((Vector2)(ref val6)).Length();
				}
				if (num9 <= num)
				{
		...
		foreach (Powerup powerup in oracle.GetPowerups())
		{
			...
			if (flag3)
			{
				foreach (PlayerShip ship in oracle.GetShips())
				{
					if (ship.wantsToTakePowerup(powerup))
					{
						...
						float num11 = ((Vector2)(ref val7)).LengthSquared();
						Vector2 val8 = base.Position - powerup.Position;
						if (num11 < ((Vector2)(ref val8)).LengthSquared() && !isConnectedWith(ship))
						{
							flag3 = false;
						}
					}
			...
			float num12 = ((Vector2)(ref val9)).Length();
			Vector2 val10 = position - base.Position;
			if (num12 < ((Vector2)(ref val10)).Length())
			{
				position = powerup.Position;
			}
			Vector2 val11 = powerup.Position - base.Position;
			if (((Vector2)(ref val11)).Length() <= num)
			{
				Vector2 val12 = powerup.Position - base.Position;
				float num13 = MyMath.PowerCurve(num3, num2, 2f, ((Vector2)(ref val12)).Length() / num);
				if (flag2)
				{
					Vector2 val13 = powerup.Position - base.Position;
					num13 = MathHelper.Lerp(num3, num2, ((Vector2)(ref val13)).Length() / num);
				}
		...
		if (position.X > 2000f && !collection.ContainsType<Floor>() && connectors.Count == 0)
		{
			if (oracle.LiveShips == 1)
			{
				...
				{
					((Vector2)(ref position))._002Ector(400f, 400f);
				}
			}
			else if (collection.ContainsType<Wall>())
			{
				float num14 = 800 / (oracle.Players + 1);
				((Vector2)(ref position))._002Ector((float)(player + 1) * num14, 300f);
			}
			else
			{
				float num15 = 800 / (oracle.Players + 1);
				((Vector2)(ref position))._002Ector((float)(player + 1) * num15, 400f);
			}
		}
		if (position.X > 2000f && collection.ContainsType<Floor>() && connectors.Count == 0)
		{
			if (oracle.LiveShips == 1)
			{
				((Vector2)(ref position))._002Ector(266f, 300f);
			}
			else
			{
				((Vector2)(ref position))._002Ector(266f, 600f / (float)(oracle.Players + 1) * (float)(player + 1));
			}
		}
		if (position.X < 2000f)
		{
			val = base.Position - position;
			float num16 = ((Vector2)(ref val)).Length();
			if (num16 > 10f)
			{
		...
		if (((Vector2)(ref direction)).Length() <= 0.2f)
		{
	...
	private void FireAt(float direction)
	{
		...
		{
			shoottimer.Start();
			Bullet bullet = Bullet.NewBullet(collection, ((GameComponent)this).Game);
	...
	private void DoSpecial(bool pickup)
	{
		...
		case Powerup.PowerupType.Option:
		{
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					Option option = Option.NewOption(collection, ((GameComponent)this).Game);
	...
	private void doPowerupEffect()
	{
		...
		powerupEffect = PowerupEffect.NewPowerupEffect(collection, ((GameComponent)this).Game);
	...
	public override void CollidesWith(ICollidable other)
	{
		...
		{
			ShipConnector shipConnector = ShipConnector.NewAlien(collection, ((GameComponent)this).Game);
	...
	private void AsplodeWall()
	{
		...
		float impulse = ((Vector2)(ref backgroundSpeed)).Length();
