...
public class Quad
{
	...
	public void LoadContent()
	{
		...
		if (!alreadyloaded)
		{
			...
			topeffect = new BasicEffect(graphicsDevice, (EffectPool)null);
			topeffect.World = Matrix.Identity;
			topeffect.View = view;
			topeffect.Projection = projection;
			topeffect.TextureEnabled = true;
			topeffect.Texture = top;
			bottomeffect = new BasicEffect(graphicsDevice, (EffectPool)null);
	...
	public void Draw()
	{
		...
		GraphicsDevice graphicsDevice = ServiceHelper.Get<IGraphicsDeviceService>().GraphicsDevice;
		BlendFunction blendFunction = graphicsDevice.RenderState.BlendFunction;
		graphicsDevice.VertexDeclaration = vertexdecl;
		graphicsDevice.RenderState.BlendFunction = (BlendFunction)1;
		graphicsDevice.RenderState.SourceBlend = (Blend)5;
		graphicsDevice.RenderState.DestinationBlend = (Blend)2;
		graphicsDevice.RenderState.AlphaBlendEnable = true;
		((Effect)effect).Begin();
		foreach (EffectPass pass in ((Effect)effect).CurrentTechnique.Passes)
		{
			pass.Begin();
			graphicsDevice.DrawUserIndexedPrimitives<VertexPositionNormalTexture>((PrimitiveType)4, vertices, 0, 4, indices, 0, 2);
			pass.End();
		}
		((Effect)effect).End();
		((Effect)topeffect).Begin();
		foreach (EffectPass pass2 in ((Effect)topeffect).CurrentTechnique.Passes)
		{
			pass2.Begin();
			graphicsDevice.DrawUserIndexedPrimitives<VertexPositionNormalTexture>((PrimitiveType)4, topvertices, 0, 4, indices, 0, 2);
			pass2.End();
		}
		((Effect)topeffect).End();
		((Effect)bottomeffect).Begin();
		foreach (EffectPass pass3 in ((Effect)bottomeffect).CurrentTechnique.Passes)
		{
			pass3.Begin();
			graphicsDevice.DrawUserIndexedPrimitives<VertexPositionNormalTexture>((PrimitiveType)4, bottomvertices, 0, 4, indices, 0, 2);
			pass3.End();
		}
		((Effect)bottomeffect).End();
		graphicsDevice.RenderState.BlendFunction = blendFunction;
	}
	...
	private Vector3 convertToVector3(float direction)
	{
		...
		((Vector2)(ref val))._002Ector(Convert.ToSingle(Math.Cos(direction)), -1f * Convert.ToSingle(Math.Sin(direction)));
