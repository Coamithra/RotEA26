...
public class ScreenshotSaver
{
	...
	public static void SaveScreenShot(Texture2D Screenshot, Levels level)
	{
		...
		lock (Savable.syncObj)
		{
			...
			RenderTarget2D val = new RenderTarget2D(graphicsDevice, (int)SIZE.X, (int)SIZE.Y, 0, graphicsDevice.PresentationParameters.BackBufferFormat);
	...
	public static void LoadScreenshot(Levels level)
	{
		...
		try
		{
			...
			binaryReader.Close();
			val2 = new Texture2D(graphicsDevice, (int)SIZE.X, (int)SIZE.Y, 1, (TextureUsage)0, graphicsDevice.PresentationParameters.BackBufferFormat);
