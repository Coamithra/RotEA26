...
internal class Spider : KillableAlien
{
	...
	protected override void KilledBy(ICollidable other, bool isComboGenerator)
	{
		...
		bloodExplosion2.Setup(position, 5f, 0.75f, MathHelper.Min(((Vector2)(ref val)).Length(), 0.24f), MyMath.VectorToAngle(oracle.BackgroundSpeed + new Vector2(0f, yspeed)));
		...
		bloodExplosion3.Setup(position2, 3f, 0.5f, MathHelper.Min(((Vector2)(ref val2)).Length(), 0.24f), MyMath.VectorToAngle(oracle.BackgroundSpeed + new Vector2(0f, yspeed)));
		bloodExplosion.MakeGreen();
		collection.Add((GameComponent)(object)bloodExplosion);
		Die();
