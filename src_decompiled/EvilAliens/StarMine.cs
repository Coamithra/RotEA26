	...
	public override void Update(GameTime gameTime)
	{
		...
		case MineState.free:
		{
			...
			foreach (PlayerShip ship in oracle.GetShips())
			{
				...
				if (((Vector2)(ref val2)).LengthSquared() <= num * num)
				{
					target = ship;
					flag = true;
				}
		...
		case MineState.attracted_to_player:
		{
			...
			if (((Vector2)(ref val3)).LengthSquared() > 0.25f)
			{
			...
			if (((Vector2)(ref val4)).LengthSquared() >= num2 * num2)
			{
				state = MineState.free;
				connectToBackground();
			}
		...
		case MineState.attracted_to_boss:
		{
			...
			if (((Vector2)(ref val)).LengthSquared() > 0.25f)
			{
