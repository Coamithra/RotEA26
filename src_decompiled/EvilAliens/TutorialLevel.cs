...
internal class TutorialLevel : GameScene
{
	...
	protected override void PopulateEventList()
	{
		...
		SingleEnemySpawner gameEvent = new SingleEnemySpawner(((GameComponent)this).Game);
		...
		wait(3f);
		waitEvent = new WaitEvent(((GameComponent)this).Game, 0.01f);
		eventList.AddEvent(waitEvent);
		waitEvent.OnFinished += spawnPunchingBag;
		PowerUpTrainingEvent gameEvent2 = new PowerUpTrainingEvent(((GameComponent)this).Game);
		...
		UnlockEvent unlockEvent = new UnlockEvent(((GameComponent)this).Game, "Evil Aliens Classic", Unlockables.Items.ClassicAliens, AnimatedMessage.UnlockType.challenge, level);
	...
	{
		PunchingBag component = PunchingBag.NewPunchingBag(Collection, ((GameComponent)this).Game);
	...
	{
		BonusUFOSpawner gameEvent = new BonusUFOSpawner(((GameComponent)this).Game, 4f, 1.5f, powerup);
		eventList.AddEvent(gameEvent);
		eventList.AddHalt();
	}
	...
	private void messageEvent_OnFinished(GameEvent sender)
	{
		...
		Asteroid asteroid = Asteroid.NewAsteroid(Collection, ((GameComponent)this).Game);
