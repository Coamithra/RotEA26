...
internal class Wall : AlienDrawableGameComponent
{
	...
	public void Setup(int variation)
	{
		...
		base.Speed = ((Vector2)(ref backgroundSpeed)).Length() * 1f;
	}
		...
		for (int i = 0; i < height; i++)
		{
			...
			for (int j = 0; j < width; j++)
			{
				if (blocks[i, j])
				{
					...
					((Vector2)(ref val2))._002Ector((float)texture.Width * scale / 2f);
					val += val2;
					((Color)(ref val3))._002Ector(new Vector4(0f, 0f, 0f, 0.6f));
					((Color)(ref val4))._002Ector(new Vector4(1f, 1f, 1f, 0.3f));
					if (isfree(j + 1, i))
					{
