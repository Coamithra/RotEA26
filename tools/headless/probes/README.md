...
| `harness_run.txt` / `harness_run_absent.txt` | `?harnessrun` lifts the sprite harness' freeze, and WITHOUT it the harness is still frozen (card d1ee8761). A pair, and read as one: alone, neither says the flag is what made the difference. The subject is the respawn summon, the object the seam was extracted from; the absent leg asserts the frozen clock as exact VALUES (`fill=0.000 secs=10 remainMs=10000`) at two marks rather than as an absence, because "the fill did not move" is also true of a boot that never built the object |
| `harness_run_generic.txt` / `harness_run_generic_absent.txt` | the same seam on `bullet` -- a key that never had a run variant, of an unrelated type -- because a seam shown only on the object it was extracted from has not been shown to generalise (card d1ee8761). The observable is the COMPONENT CENSUS, which is type-independent: a running Bullet flies off and Die()s, a frozen one is parked forever. **This pair is the only one that catches the third enforcement site** (the Update dispatch chain re-parking `Position`), since a respawn summon never moves, so it is not redundant with the pair above. `bullet` rather than an enemy with a firing diagnostic on purpose: EvilSkull's rearm interval comes off the unseeded `RandomHelper` and flaked 2 of 6 |
...
return early drops the `scene speed=` line while the `dump v3 ... end` sentinel still passes, which
