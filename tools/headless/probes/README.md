...
entirely, and keeping the log while passing a `null` source rect prints `src=full` — both fail the
same `src=10x10` expect. **Rebuild `tools/headless` after either**, or the probe tests the old
