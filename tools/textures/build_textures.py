...
def write_manifest_cs(entries, dry):
    ...
    # Content-relative path. dxt -> ".dds", raw -> ".rtex" (each asset ships exactly one).
    lines = []
    for e in entries:
        asset = e[1]
        ext = ".dds" if e[0] == "dxt" else ".rtex"
        lines.append(f'            {{ "Content/{asset}", "{ext}" }},')
    body = "\n".join(lines)
    text = (
        "// <auto-generated>\n"
    ...
    print(f"  manifest  {len(entries)} entr{'y' if len(entries)==1 else 'ies'} -> "
