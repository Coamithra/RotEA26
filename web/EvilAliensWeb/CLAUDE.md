...
- **The SpiderBoss fight is scripted, so its counters are too** (unashamedly special-cased -- it
  ...
  dodging a sweep and a beam at once is what kills it. `?aibigufopx=<px>` overrides the radius
  ...
  and 1.83 -> 2.00 alive per boss death). 200px (+1.25 +- 0.70 deaths) and 300px (fights mostly
  unresolved, 6/16 runs saw a boss death) both read worse than 250 at N=16, so 250 is an
  interior pick, not a gradient's edge. Its three fixed lanes and its hard-coded X-600 landing column are avoided for the WHOLE
