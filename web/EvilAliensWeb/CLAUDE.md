...
- **A pass that walks a live collection must FREEZE its count first.** Instant births mean any
  ...
  **The contract is PINNED by `eaBinTest()`** (card bcdc7430), one scenario per loop the fix
  touched: a `CollisionMultibox` scratch alien spawning from the fill phase all-pairs branch,
  and a box pair spawning from the resolution loop. Neither leans on the throw -- whether
  `boxes[m]` goes out of range depends on the high-water mark `boxes` accumulated from prior
  play, so a "does it throw" test would pass on the broken code and its verdict would be a
  function of session history. Each instead PLANTS the fault precondition: 5a relies only on
  `List<T>` version-checking its enumerator, and 5b runs a warm-up pass plus a filler
  collidable it then removes, so the newborn lands exactly on a stale `boxes` entry the clear
  loop skipped. Both carry a positive control, so a green run cannot be vacuous. Verified by
  reverting `DetectCollisions` to its pre-fix form: 5a reports `InvalidOperationException` and
  5b reports the newborn participating, in the menu AND mid-level.
- **Diagnostics:** `?binlog` logs filter diverts + pause-frozen adds, and reports how many
  ...
  the live bin and prints PASS/FAIL (20 scenarios: the lifecycle ones plus the
  collision-pass contract in the bullet above). `eaKillShips()` asplodes every locally-owned `PlayerShip`
