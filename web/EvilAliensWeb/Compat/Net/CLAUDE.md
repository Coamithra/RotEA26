...
- **FINISHING A LEVEL RETURNS BOTH PEERS TO THE LOBBY WITH THE SESSION ALIVE (card 3b6c12e7).**
  ...
  - **AND THE SURVIVING LISTED SESSION IS CONVERTED: `menuSession = true; listedSession = false`.**
    ...
    *"The other player left / Match ended"* for anything else, so leaving the flag set would
    ...
    level is gone. The `?net=jiphost` dev re-arm is untouched -- it fires off `!Active`, which this
  ...
  - **Verified by `tools/headless/probes/net_level_end_listed.txt`** (`NetLevelEndTest.ArmListed`
    ...
    not from pairing once it has been). Mutation-tested: the old guard fails 6 of its 13 legs.
...
- **THE MATCH-END POLICY (the card's design decision, adopted as proposed and UNIFORM, N=2
  ...
  **Consequence at N=2, deliberate:** a menu-session host whose partner drops now keeps playing
  ...
  thing that ends a match with the host still present is somebody deliberately leaving it**: a
  quit or force-exit (`FinishedMode.exit`), a kick, or a reject.
