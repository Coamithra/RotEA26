...
- **Protocol (`Compat/Net/NetProtocol`, little-endian binary, 1-byte type, v10):** the 3
  ...
  **v10** adds `EvCosmeticSwarm` -- a decorative swarm replicates as one on/off beat and its
  ...
  says why that is a decision and not an oversight. Card 8a7772d6 adds **`ShipFlagScriptGate`**
  (a spare bit of `MsgShipState`'s existing flags byte -- no width change) and **`EvIntroVolley`**
  (event 23) and stays on v10 for the same reason; see the intro-cinematic bullet under
  "World authority").
...
- **LEVEL 1'S INTRO CINEMATIC RUNS ON BOTH PEERS (card 8a7772d6).** Level 1 opens with ~10.5 s of
  ...
  - **PART B, the volley: `EvIntroVolley` (event 23, reliable, `[seed:4]`).** `Bullet` is NOT in
