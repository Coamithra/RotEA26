...
namespace EvilAliensWeb.Compat.Net
{
    ...
    //
    // THE FIX IS IN PACKETS, WHICH IS WHY LEG 1 IS SHAPED THE WAY IT IS. The peer holds the
    // newest sample until a newer one arrives, so N marked packets = N SEND INTERVALS of
    // firing=true over there, not N ms. FiringHoldMsFor derives the packet count that fits inside
    // one cadence period and converts back. A fraction-of-the-period hold in plain ms is NOT
    // equivalent and was the first thing tried: 0.6 * the 62.5 ms period at 16 shots/s is 37.5 ms,
    // which still catches TWO 33 ms-apart sends -- the same doubled tap at a rate nobody plays at
    // by accident. Leg 1 is what caught that, so it asserts the packet property over the WHOLE
    // 1..18 domain rather than spot-checking the default.
    //
    // WHAT EACH LEG IS FOR.
    //   1  the contract, as a pure decision: worst-case marked packets * the send interval must
    //      stay under one cadence period, for every reachable shotsPerSec. The PRE-CARD flat
    //      150 ms hold is computed beside it as the negative control and must VIOLATE it -- so a
    //      predicate that had degenerated into "always true" cannot pass this leg.
    ...
    //      (5 marked packets instead of 3) and must report 2 bullets. If leg 4 ever reports 1,
    ...
    //
    // LEG 1 IS THE RIGOROUS HALF AND LEGS 2-4 ARE PHASE SAMPLES -- MEASURED, not asserted. The
    // rig aligns each packet to a tick boundary, so the end-to-end legs see ONE phase of the
    // tap-vs-send-vs-cadence alignment, and a hold that is only marginally too long can land on
    // the safe side of it. Mutation-tested: the 0.6-fraction hold this card first tried (37.5 ms
    // against the 62.5 ms period at 16 shots/sec) fails leg 1 at 16/s while legs 2-5 ALL PASS,
    // including the 18/s one -- its second bullet falls one tick outside the burst. So do not
    // read a green leg 3 as covering the top of the range; leg 1 is what covers it.
    //
    ...
    internal static class NetFireTest
    {
        ...
        {
            sb.Append(" 1. the hold-window contract over every reachable fire rate\n");
            int violations = 0;
            ...
            for (int shots = 1; shots <= 18; shots++)
            {
                ...
                if (MarkedWindowMs(NetSession.FiringHoldMsFor(shots)) >= period)
                {
                    violations++;
                    if (firstViolation == 0)
                    {
                        firstViolation = shots;
                    }
                }
                if (MarkedWindowMs(PreCardHoldMs) >= period)
                {
                    preCardViolations++;
                }
            }
            Check("no fire rate in 1..18 marks enough packets to span a cadence period"
                + (violations == 0 ? "" : " -- first violation at " + firstViolation + "/s"),
                violations == 0);
            ...
            Check("NEGATIVE the pre-card flat " + PreCardHoldMs + " ms hold DOES span one"
                + " (violates at " + preCardViolations + " of 18 rates)", preCardViolations > 0);
            Check("... and it violates at the DEFAULT 8 shots/sec -- the reported case",
                MarkedWindowMs(PreCardHoldMs) >= 1000f / 8f);
            ...
            Check("every fire rate still marks at least one packet, so no tap goes unsent"
                + " (unsendable=" + unsendable + ")", unsendable == 0);
            sb.Append("    holds: 8/s=" + NetSession.FiringHoldMsFor(8) + "ms ("
                + MarkedPackets(NetSession.FiringHoldMsFor(8)) + " packets), 18/s="
                + NetSession.FiringHoldMsFor(18) + "ms ("
                + MarkedPackets(NetSession.FiringHoldMsFor(18)) + " packets)\n");
        }

        // Worst case (the tap landing the instant after a send): sends are >= StreamIntervalMs
        // apart by construction, so a hold of h ms marks ceil(h / interval) of them.
        private static int MarkedPackets(float holdMs)
        {
            return Math.Max(1, (int)Math.Ceiling(holdMs / NetSession.StreamIntervalMs));
        }

        // ...and the peer sees firing=true for that many send intervals, because it holds the
        // newest sample until the next one lands. This is the quantity the cadence period has to
        // beat, and the quantity a plain ms hold gets wrong.
        private static float MarkedWindowMs(float holdMs)
        {
            return MarkedPackets(holdMs) * (float)NetSession.StreamIntervalMs;
        }
            ...
            ComponentBin bin, Game game, PinnedNetHost clock)
        {
            ...
            Deliver(peer, wire, clock, ref shipSeq, ref shipMs, firing: false, shotsPerSec: 8);
            ...
            int fired = Tap(peer, wire, bin, game, clock, puppet, peerSlot, ref shipSeq, ref shipMs,
                shotsPerSec: 8, markedPackets: MarkedPackets(NetSession.FiringHoldMsFor(8)));
            ...
            fired = Tap(peer, wire, bin, game, clock, puppet, peerSlot, ref shipSeq, ref shipMs,
                shotsPerSec: 18, markedPackets: MarkedPackets(NetSession.FiringHoldMsFor(18)));
            ...
            fired = Tap(peer, wire, bin, game, clock, puppet, peerSlot, ref shipSeq, ref shipMs,
                shotsPerSec: 8, markedPackets: MarkedPackets(PreCardHoldMs));
            ...
            const int burstPackets = 30;   // ~1 s at the 33 ms send cadence
            fired = Tap(peer, wire, bin, game, clock, puppet, peerSlot, ref shipSeq, ref shipMs,
                shotsPerSec: 8, markedPackets: burstPackets);
            // ~990 ms of firing at 125 ms per shot. Bounded rather than pinned to a single number:
            // the tick/packet phase decides whether the last slot lands inside the burst.
        ...
        private static int Tap(InMemoryTransport peer, NetWire wire, ComponentBin bin, Game game,
            ...
            ref uint shipMs, int shotsPerSec, int markedPackets)
        {
            // Settle first: the previous leg's burst may have left the cadence gate mid-period,
            // which would move this leg's count. Enough clear packets to outlast the slowest
            // period in range, and the sweep below starts the count from a clean world.
            for (int i = 0; i < 6; i++)
            {
                Deliver(peer, wire, clock, ref shipSeq, ref shipMs, false, shotsPerSec);
            ...
            {
                Deliver(peer, wire, clock, ref shipSeq, ref shipMs, true, shotsPerSec);
                DriveTicks(puppet, bin);
            }
        ...
        private static void Deliver(InMemoryTransport peer, NetWire wire, PinnedNetHost clock,
            ref ushort shipSeq, ref uint shipMs, bool firing, int shotsPerSec)
        {
            shipMs += (uint)NetSession.StreamIntervalMs;
            ...
            wire.Pump();
            clock.Advance(NetSession.StreamIntervalMs);
            NetSession.Update();
        }
