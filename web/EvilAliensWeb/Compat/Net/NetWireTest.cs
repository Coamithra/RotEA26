        ...
        private static void SectionCodecs(Action<string, bool> check)
        {
                ...
                out ShipSample s, out int sps, out float blife))
            {
                shipOk = shipSeq == 4242 && s.T == 1234567.0
                    ...
                    && Near(s.Aim, 1.75f) && s.Alive && s.Firing && !s.ScriptGate
                    && sps == 17 && Near(blife, 640f);
            }
            check("MsgShipState round-trips every field", shipOk);

            // Card 8a7772d6's script-gate bit. It shares the flags byte with alive/firing, so
            // the leg above pins it CLEAR while both neighbours are set, and this one pins it
            // SET while both neighbours are clear -- either half alone passes a bit wired to
            // the wrong mask.
            byte[] gated = Round(NetProtocol.EncodeShipState(
                1, 5u, Vector2.Zero, Vector2.Zero, 0f, alive: false, firing: false,
            ...
            {
                gateOk = gs.ScriptGate && !gs.Alive && !gs.Firing;
            }
            check("MsgShipState carries the script-gate flag independently of alive/firing", gateOk);
