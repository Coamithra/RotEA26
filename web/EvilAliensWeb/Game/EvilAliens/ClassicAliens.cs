...
internal class ClassicAliens : GameScene
{
	...
	protected override void PopulateEventList()
	{
		...
		WaitEvent beatWait = new WaitEvent(base.Game, 0.01f);
		eventList.AddEvent(beatWait);
		eventList.AddHalt();
		eventList.SetLastEventAsCheckPoint();
		for (int i = 1; i < 21; i++)
		{
			ClassicSpawner classicSpawner = new ClassicSpawner(base.Game, i);
			...
			if (i % 4 == 0 && i < 20)
			{
				...
				MessageEvent messageEvent = new MessageEvent(base.Game, "Wave Completed!", SoundManager.Texts.WaveCompleted);
				...
				beatWait = new WaitEvent(base.Game, 2f);
				eventList.AddEvent(beatWait);
				eventList.AddHalt();
				eventList.MakeConditional(beatWait, Settings.DifficultyLevel.Very_Hard, Settings.DifficultyLevel.Inzane);
			}
			if (i > 10 && i <= 15)
			{
		...
		WaitEvent waitEvent = new WaitEvent(base.Game, 4f);
