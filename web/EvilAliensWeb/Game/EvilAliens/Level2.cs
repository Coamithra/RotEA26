...
internal class Level2 : GameScene
{
	...
	protected override void PopulateEventList()
	{
		...
		WaitEvent marsWaveGate = new WaitEvent(base.Game, 0.1f);
		marsWaveGate.LinkWith(ufoSpawner);
		...
		marsWaveGate.LinkWith(bonusSpawner);
		...
		Wait(3f);
		StationaryWave(1f, 6f, 1f, 1f, 0f, 0f);
		eventList.AddEvent(marsWaveGate, halting: false);
		...
		Wait(5f);
		UnlockEvent unlockEvent = new UnlockEvent(base.Game, "Paratroopers", Unlockables.Items.Paratrooper, AnimatedMessage.UnlockType.challenge, level);
		...
		eventList.AddHalt();
		Wait(2f);
		WaitEvent beesWait = Wait(0.1f);
		beesWait.OnFinished += beesSoundOn;
		eventList.SetLastEventAsCheckPoint();
		FlyingSpiderEvent backgroundSpiderEvent = new FlyingSpiderEvent(base.Game, 0f, 5.5f, isbackground: true);
		eventList.AddEvent(backgroundSpiderEvent, halting: false);
		Wait(4f);
		FlyingSpiderEvent foregroundSpiderEvent = new FlyingSpiderEvent(base.Game, 0f, 2f, isbackground: false);
		eventList.AddEvent(foregroundSpiderEvent, halting: false);
		Wait(2.5f);
		...
		sweepUFOSpawner.LinkWith(foregroundSpiderEvent);
		eventList.AddHalt();
		Wait(1f);
		messageEvent = new MessageEvent(base.Game);
		eventList.AddEvent(messageEvent);
		beesWait = Wait(5f);
		beesWait.LinkWith(backgroundSpiderEvent);
		eventList.SetLastEventAsCheckPoint();
		...
		Wait(2f);
		unlockEvent = new UnlockEvent(base.Game, "Base Pressure", Unlockables.Items.OwnLevel, AnimatedMessage.UnlockType.challenge, level);
		eventList.AddEvent(unlockEvent, halting: true);
		eventList.AddHalt();
		unlockEvent = new UnlockEvent(base.Game, "Turbo", Unlockables.Items.Turbo, AnimatedMessage.UnlockType.cheat, level);
		eventList.AddEvent(unlockEvent, halting: true);
		eventList.AddHalt();
		unlockEvent = new UnlockEvent(base.Game, "Next Mission!", Unlockables.Items.Level3, AnimatedMessage.UnlockType.level, level);
		eventList.AddEvent(unlockEvent, halting: true);
		eventList.AddHalt();
