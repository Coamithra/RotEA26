...
internal class Level2 : GameScene
{
	...
	protected override void PopulateEventList()
	{
		...
		WaitEvent marsWaveLink = new WaitEvent(base.Game, 0.1f);
		marsWaveLink.LinkWith(ufoSpawner);
		...
		marsWaveLink.LinkWith(bonusSpawner);
		...
		Wait(3f);
		StationaryWave(1f, 6f, 1f, 1f, 0f, 0f);
		eventList.AddEvent(marsWaveLink, halting: false);
		...
		eventList.SetLastEventAsCheckPoint();
		FlyingSpiderEvent backgroundSpiders = new FlyingSpiderEvent(base.Game, 0f, 5.5f, isbackground: true);
		eventList.AddEvent(backgroundSpiders, halting: false);
		Wait(4f);
		FlyingSpiderEvent foregroundSpiders = new FlyingSpiderEvent(base.Game, 0f, 2f, isbackground: false);
		eventList.AddEvent(foregroundSpiders, halting: false);
		Wait(2.5f);
		...
		sweepUFOSpawner.LinkWith(foregroundSpiders);
		eventList.AddHalt();
		Wait(1f);
		messageEvent = new MessageEvent(base.Game);
		eventList.AddEvent(messageEvent);
		beesWait = Wait(5f);
		beesWait.LinkWith(backgroundSpiders);
		eventList.SetLastEventAsCheckPoint();
